...
$(function () {
    ...
    $('#btn').on('click', function () {
        if (clickNum >= 1) {
            ...
            rotateNum++;
            //“开始抽奖”按钮无法点击恢复点击 setTimeout函数表示一段时间后调用该函数
            setTimeout(function () {
                alert(notice);
    ...
    function probability() {
        ...
        switch (num) {
            case 0:
                angles = 2160 * rotateNum + 1800;
                notice = info[0] + info1[0];
                break;
            case 1:
                angles = 2160 * rotateNum + 1845;
                notice = info[7] + info1[7];
                break;
            case 2:
                angles = 2160 * rotateNum + 1890;
                notice = info[6] + info1[6];
                break;
            case 3:
                angles = 2160 * rotateNum + 1935;
                notice = info[5] + info1[5];
                break;
            case 4:
                angles = 2160 * rotateNum + 1980;
                notice = info[4] + info1[4];
                break;
            case 5:
                angles = 2160 * rotateNum + 2025;
                notice = info[3] + info1[3];
                break;
            case 6:
                angle = 2160 * rotateNum + 2070;
                notice = info[2] + info1[2];
                break;
            case 7:
                angles = 2160 * rotateNum + 2115;
    ...
    function circle() {
        ...
        function createCricle() {
            ...
            for (let i = 0; i < 8; i++) {
                ...
                ctx.save(); //保存当前绘制状态
                ctx.beginPath();//开始一个新的形状描述，创建路径之前，必须先调用这个方法
                //绘制圆弧arc(x, y, 半径, startangle, endangle, 顺时针 true/逆时针 false)
                ctx.arc(150, 150, 100, startAngle, endAngle, false);
                //增大画笔宽度，圆弧变成圆环
                ctx.lineWidth = 120;
                i % 2 == 0 ? (ctx.strokeStyle = color[0]) : (ctx.strokeStyle = color[1]);
                ctx.stroke();//将路径绘制为轮廓形状
                ctx.restore();//恢复前一次绘制状态
            }
        }
        ...
        function createCirText() {
            ...
            for (let i = 0; i < 8; i++) {
                ctx.save();
                ctx.beginPath();
                ctx.translate(150, 150);//重新映射画布上的 (x,y) 位置。
                ctx.rotate(i * step);//旋转
                ctx.font = " 20px Microsoft YaHei";//字体
                ctx.fillStyle = color[3];//颜色
                ctx.fillText(info[i], -30, -115, 60);//填充文字
